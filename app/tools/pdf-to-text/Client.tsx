...
import { pdfToText } from "@/lib/pdfTools";
...
export default function PdfToTextPage() {
  ...
  async function handleConvert() {
    ...
    try {
      ...
      if (!text.trim()) throw new Error("Couldn't find any selectable text in this PDF — scanned pages need OCR first.");
      setResult(text);
    } catch (e: any) {
      ...
      <div className="mx-auto max-w-xl px-6 mt-8">
        {result ? (
          <ResultPanel
            ...
            onDownload={() => downloadBytes(new TextEncoder().encode(result), `${stripExt(file!.name)}.txt`, "text/plain")}
            onReset={reset}
          />
        ) : !file ? (
          ...
          <div className="paper-stack p-6 text-center">
            ...
            <p className="mt-3 text-xs text-ink-faint">
              Text is read out in the order it appears on the page, with line breaks kept — no formatting, images, or
              layout carry over. For a scanned PDF with no selectable text, run OCR PDF first.
            </p>

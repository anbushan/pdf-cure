...
import { LucideIcon, LogOut, Menu, X, ExternalLink } from "lucide-react";
...
/**
 * Shared sidebar/top-bar shell behind both /admin and /account — same
 * slide-in mobile drawer, desktop sidebar, and logout confirmation, just
 * parameterized by nav items and branding so the two don't duplicate
 * ~170 lines of near-identical layout code.
 */
export default function DashboardShell({ children, navItems, brandLabel, baseHref, userEmail, userName, logoutMessage }: DashboardShellProps) {
  ...
  const ProfileCard = () => (
    <div className="flex items-center gap-2.5 rounded-md border border-paper-line bg-paper-dim/60 px-3 py-2.5">
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-medium leading-tight text-ink">{userName || "Account"}</p>
      ...
      <div className="sticky top-0 z-40 border-b border-paper-line bg-paper/95 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-3 sm:px-6">
          <div className="flex items-center gap-3">
            ...
            <Link href={baseHref} className="font-display text-base font-semibold tracking-tight text-ink">
              PDF<span className="text-rust">Cure</span> <span className="text-ink-faint font-normal">{brandLabel}</span>
            </Link>
          </div>

          <div className="flex items-center gap-3">
            <Link href="/" className="hidden items-center gap-1.5 text-sm text-ink-faint hover:text-ink sm:flex">
              View site <ExternalLink size={13} />
            </Link>
            <div className="hidden text-right sm:block">
              <p className="text-sm font-medium text-ink leading-tight">{userName || "Account"}</p>
